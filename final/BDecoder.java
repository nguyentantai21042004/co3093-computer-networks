...
public class BDecoder {
     ...
     * byte array.
     *
     * @throws Exception If it is not a b-encoded byte array.
     */
    public BEValue bdecodeBytes() throws Exception {
        int c = this.getNextIndicator();
        int num = c - '0';
        if (num < 0 || num > 9)
            throw new Exception("Number expected, not '"
                    + (char) c + "'");
        this.indicator = 0;
        ...
        if (c != ':') {
            throw new Exception("Colon expected, not '" +
                    (char) c + "'");
        }
     ...
     * number.
     *
     * @throws Exception If it is not a number.
     */
    public BEValue bdecodeNumber() throws Exception {
        int c = this.getNextIndicator();
        if (c != 'i') {
            throw new Exception("Expected 'i', not '" +
                    (char) c + "'");
        }
        this.indicator = 0;
        ...
        if (c == '0') {
            ...
            if (c == 'e')
                ...
                throw new Exception("'e' expected after zero," +
                        " not '" + (char) c + "'");
        }
        ...
        if (c < '1' || c > '9')
            throw new Exception("Invalid Integer start '"
                    + (char) c + "'");
        chars[off] = (char) c;
        off++;
     ...
     * list.
     *
     * @throws Exception If it is not a list.
     */
    public BEValue bdecodeList() throws Exception {
        int c = this.getNextIndicator();
        if (c != 'l') {
            throw new Exception("Expected 'l', not '" +
                    (char) c + "'");
        }
        this.indicator = 0;
     ...
     * map (dictionary).
     *
     * @throws Exception If it is not a map.
     */
    public BEValue bdecodeMap() throws Exception {
        int c = this.getNextIndicator();
        if (c != 'd') {
            throw new Exception("Expected 'd', not '" +
                    (char) c + "'");
        }
        this.indicator = 0;
